...
//Custom function download image
//void ST7735_DrawImage_Card(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
//{
//	uint16_t buff[1] = {0};
//
//    if((x >= _width) || (y >= _height)) return;
...
//    TFT_CS_L();
//    ST7735_SetAddressWindow(x, y, x+w-1, y+h-1);
//    for(uint32_t addr = 0; addr < 0xA000; addr+=2)
//    {
//    	buff[0] = Send_READ_command_2B(addr);
//    	HAL_SPI_Transmit(&hspi2, buff[0], 2, 100);
//    }
//    TFT_CS_H();
//}
